...
export const decodePayReq = (payReq, addDefaults = true) => {
  ...
  if (addDefaults && !expiry) {
    ...
    data.timeExpireDateString = new Date(
      data.timeExpireDate * 1000
    ).toISOString();
  }
  return data;
};
  ...
  try {
    if (payReq) {
      ...
      const descriptionTag =
        request.tags.find(tag => tag.tagName === 'description') || {};
...
export const getMinFee = (routes = []) => {
  ...
  const fee = routes.reduce(
    (min, b) => Math.min(min, b.total_fees),
    routes[0].total_fees
  );
...
export const getMaxFee = routes => {
  ...
  const fee = routes.reduce(
    (max, b) => Math.max(max, b.total_fees),
    routes[0].total_fees
  );
...
export const getMaxFeeInclusive = routes => {
  ...
  const retryCount = 2;
  const feeIncrementExponent = 1.1;

  const fee = getMaxFee(routes);
  return range(retryCount).reduce(
    max => Math.ceil(max * feeIncrementExponent),
    fee
  );
};
