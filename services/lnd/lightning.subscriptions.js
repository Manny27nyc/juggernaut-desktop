...
import { forwardAll } from '../../utils/events/events';
...
function subscribeChannelEvents(payload = {}) {
  const call = this.service.subscribeChannelEvents(payload);
  call.on('data', data => {
    grpcLog.debug('CHANNELEVENT DATA: %o', data);
    this.emit('subscribeChannelEvents.data', data);
  });
  call.on('error', error => {
    if (error.code !== status.CANCELLED) {
      grpcLog.error('CHANNELEVENT ERROR: %o', error);
      this.emit('subscribeChannelEvents.error', error);
    }
  });
  call.on('status', status => {
    grpcLog.info('CHANNELEVENT STATUS: %o', status);
    this.emit('subscribeChannelEvents.status', status);
  });
  call.on('end', () => {
    grpcLog.info('CHANNELEVENT END');
    this.emit('subscribeChannelEvents.end');
  });
  return call;
}

function subscribePeerEvents(payload = {}) {
  const call = this.service.subscribePeerEvents(payload);
  call.on('data', data => {
    grpcLog.debug('PEEREVENT DATA: %o', data);
    this.emit('subscribePeerEvents.data', data);
  });
  call.on('error', error => {
    if (error.code !== status.CANCELLED) {
      grpcLog.error('PEEREVENT ERROR: %o', error);
      this.emit('subscribePeerEvents.error', error);
    }
  });
  call.on('status', status => {
    grpcLog.info('PEEREVENT STATUS: %o', status);
    this.emit('subscribePeerEvents.status', status);
  });
  call.on('end', () => {
    grpcLog.info('PEEREVENT END');
    this.emit('subscribePeerEvents.end');
  });
  return call;
}
